...
public class Activity10 {
    ...
    public static List<List<Object>> readExcel(String filePath) {
        ...
        try {
			...
			/*
			 * for (Row row: sheet) { // Temp var List<Object> rowData = new ArrayList<>();
			 * // Iterate over all the cells one by one for(Cell cell: row) { if(cell !=
			 * null) { switch (cell.getCellType()) { case STRING ->
			 * rowData.add(cell.getStringCellValue());break; case NUMERIC ->
			 * rowData.add(cell.getNumericCellValue());break; case BOOLEAN ->
			 * rowData.add(cell.getBooleanCellValue());break; } } } data.add(rowData); }
			 */
            file.close();
            workbook.close();
        } catch (Exception e) {
